...
bigN=20000 #number of subjects

#populate data with five possible values
...
ATE <- mean(Y1)-mean(Y0)
print(ATE) #should be close to 1.5

PO_DF <- data.frame(Y0,Y1,X)



# RANDOMLY ASSIGN A TREATMENT AND CREATE TREATMENT STATUS INDICATOR
D <- as.numeric((runif(bigN,0,1)) > 0.5)
mean(D) #should be about 0.5 (half above and half below)
